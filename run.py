...
with open("{}/edge_text.bin".format(folder), 'rb') as f: #dict: k=(u_idx, i_idx), v={word_idx: count}
    edge_text = pkl.load(f)
# convert to doc list
docs = list()
doc_ids = list()
doc_cnt = list()
for edge,words in edge_text.items():
    doc = list()
    ids = []
    cnt = []
    for w_idx, w_cnt in words.items():
        ids.append(w_idx)
        cnt.append(w_cnt)
        for i in range(w_cnt):
            doc.append(w_idx)
    doc_ids.append(np.array(ids))
    doc_cnt.append(np.array(cnt))
...
def print_top_words(beta, feature_names, n_top_words=8):
    print ('---------------Printing the Topics------------------')
    for i in range(len(beta)):
        topic = " ".join([feature_names[j]
